...
test('simulated 3G: app remains responsive and LCP threshold is enforced in UI', async ({ browser }) => {
  ...
  await expect(page.locator('#modelSelect option')).toHaveCount(1);
...
test('voice design ASR + TTS text similarity and anti-single-tone safeguards', async ({ page, request }) => {
  ...
  const metrics = await page.evaluate(async () => {
    ...
    return {
      ...
      webSpeechTranscript: latest.webSpeechTranscript,
  ...
  expect(jaccardWordSimilarity(metrics.webSpeechTranscript, QWEN_EXAMPLE_TEXT)).toBeGreaterThan(0.9);
  ...
  const stopButton = page.locator('button[data-stop-id]').first();
  ...
  await expect(stopButton).toBeDisabled();
  await expect(page.locator('#synthStatus')).toContainText('Stopped saved audio playback');
});
